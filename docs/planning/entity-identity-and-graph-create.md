...
|---|---|---|
| **P0** | `governance/identifier-standard.md` + allowlist; `@fuzeone/shared/identity` (registry, branded `EntityId<T>`, TypeID codec, mint/parse/assertRef); `gate_identifier.py` + harden-gate wiring; spectral glob widened to `packages/*`; `additionalProperties: false` added to 7 create schemas in `packages/security` | **done** |

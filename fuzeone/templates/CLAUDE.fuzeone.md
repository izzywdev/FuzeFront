...
- **GitOps for deploy**: prod is GitOps-only (never hand-deploy); infra/cluster changes are *declared* in `deploy/` and reconciled by FuzeInfra — never edit FuzeInfra directly (delegate via a `@fuze` issue).

## Standard automation (installed by fuzeone)
`claude.yml` (@fuze handler) · `claude-auto-pr.yml` (issue→draft-PR) · `auto-merge.yml` (merge-on-green) · `claude-ci-autofix.yml` + `telegram-pr-merged.yml` (self-contained; CI-failure autofix + merge notifications) (+ `helm-validate.yml` and `infra-dispatch.yml` when this repo has a chart / declared infra). Re-run `fuzeone sync` to update the local files.

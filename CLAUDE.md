...
**Extracted (2026-08-10): new features author frames in FuzeX, not this repo.** The navigable-frames technology itself now lives as a standalone product, `design-frames-service`, in `izzywdev/FuzeX` — consumed over REST/MCP/A2A (see `docs/planning/design-first-ui-pipeline.md`'s extraction addendum and FuzeX's `services/design-frames-service/docs/EXTRACTION.md`). This repo's 13 existing `design/frames/<feature>/` directories are **frozen, untouched** — the rest of this section describes their local pipeline, which still governs *them*. `gate-frames-external` blocks any new `design/frames/<feature>/` directory not on the frozen legacy list; author new features against FuzeX instead.

**The authoritative design artifact for the 12 frozen legacy features is a set of navigable HTML frames in this repo** — `design/frames/<feature>/` (`index.html` entry + ordered `01-*.html` screens + `tokens.css` + `manifest.json`), published to GitHub Pages for review. Not PenPot. PenPot is **parked** by owner decision; it complicates the loop without earning it, and a design tool that is not in the repo cannot be gated by CI. Frames are code: they diff, they review, they enforce.

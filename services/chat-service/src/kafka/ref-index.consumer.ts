...
/**
 * Keeps chat_ref_index current from the lifecycle events the owning services
 * already publish (FFRNT P2 / identifier-standard.md §5, layer L1).
 *
 * Chat-service references userId (from JWT) and orgId (from JWT / body).
 * These entities are owned by security-service and host backend. The
...
export async function startRefIndexProjection(store: RefIndexStore): Promise<void> {
  if (!kafkaEnabled()) {
    console.log(
      '[chat-service] Kafka disabled — chat_ref_index projection not started (KAFKA_BROKERS unset)',
    );
    return;
  }
  ...
  try {
    ...
    consumer = new TypedConsumer(
      kafka,
      (process.env.KAFKA_GROUP_ID || 'chat-service') + '-ref-index',
    );
    ...
    if (rebuilding) {
      console.log(
        '[chat-service] chat_ref_index is empty — replaying lifecycle topics from the ' +
    ...
    void consumer.run(
      async (event: { topic: string; payload: Record<string, unknown> }) => {
        try {
          ...
          console.error('[chat-service] chat_ref_index apply error:', err);
        }
      },
      anyPayload,
    );
    ...
    console.log(
      `[chat-service] chat_ref_index projection consuming: ${REF_INDEX_TOPICS.join(', ')}`,
    );
  } catch (err) {
    console.warn(
      '[chat-service] Failed to start chat_ref_index projection (continuing without it):',
      err,
    );
  }
}

// migrate.ts — CLI entrypoint for the chat-db-migrate Job.
//
// Runs the knex migrations in `db/migrations` to latest and exits. Invoked by
// the Helm `chat-db-migrate` pre-install/pre-upgrade hook as
// `node dist/db/migrate.js`, mirroring how the chat-doc-indexer Job invokes
...
async function main(): Promise<void> {
  ...
  if (applied.length === 0) {
    ...
    console.log('[chat-migrate] Schema already up to date; nothing to apply.');
    return;
  }

  // eslint-disable-next-line no-console
  console.log(`[chat-migrate] Applied ${applied.length} migration(s):`);
  applied.forEach((name) => {
    // eslint-disable-next-line no-console
    console.log(`[chat-migrate]   - ${name}`);
  });
}
...
if (require.main === module) {
  main()
    ...
    .catch(async (err) => {
      // eslint-disable-next-line no-console
      console.error('[chat-migrate] Migration failed:', err);
      // Fail loudly and non-zero: the Job is a pre-upgrade hook, so a silent
      // success here would let chat-service start against a schema-less DB.

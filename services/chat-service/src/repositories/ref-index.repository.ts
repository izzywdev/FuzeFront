 ...
 *
 * Uses the `chat_ref_index` and `chat_ref_index_state` tables. Chat-service
 * references userId and orgId from JWTs and request bodies — entities owned by
 * the security-service and host backend. The projection allows assertRefExists
 * to answer without an RPC to those services.
 *
...
export class KnexRefIndexRepository implements RefIndexStore {
  ...
  private readonly table = 'chat_ref_index';
  private readonly stateTable = 'chat_ref_index_state';

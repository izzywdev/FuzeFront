...
exports.defaultPermitClient = exports.PROVISIONING_STEPS = void 0;
exports.ensurePersonalOrg = ensurePersonalOrg;
exports.reconcileOrganizationProvisioning = reconcileOrganizationProvisioning;
exports.runInternalProvision = runInternalProvision;
 ...
 */
exports.defaultPermitClient = {
    async syncUser(org, ownerEmail) {
        const ok = await (0, user_sync_1.syncUserToPermit)({
            id: org.owner_id,
            email: ownerEmail,
            roles: [],
        });
        if (!ok)
            throw new Error('syncUserToPermit returned false');
    },
    async createTenant(org) {
        // createTenantInPermit throws on real failure, returns true on success/409.
        await (0, tenant_management_1.createTenantInPermit)(org);
    },
    async assignOwnerRole(org) {
        const ok = await (0, role_assignment_1.assignOrganizationRole)(org.owner_id, org.id, 'owner');
        if (!ok)
            throw new Error('assignOrganizationRole returned false');
    },
};
function getDeps(overrides) {
    return {
        db: overrides?.db ?? database_1.db,
        permit: overrides?.permit ?? exports.defaultPermitClient,
        publish: overrides?.publish ?? eventPublisher_1.defaultEventPublisher,
    };
}
 ...
 */
async function ensurePersonalOrg(userId, overrides) {
    ...
    if (!user)
        throw new Error(`Cannot create personal org: user ${userId} not found`);
...
async function ensureStepRows(db, orgId) {
    await ensureStepRowsTrx(db, orgId);
}
// Accepts either a Knex instance or a transaction (both expose the same query API).
async function ensureStepRowsTrx(qb, orgId) {
    const rows = await qb('organization_provisioning').where({
        organization_id: orgId,
    });
    const present = new Set(rows.map((r) => r.step));
    const missing = exports.PROVISIONING_STEPS.filter(s => !present.has(s)).map(step => ({
        id: (0, uuid_1.v4)(),
        organization_id: orgId,
        step,
        status: 'pending',
...
async function runStep(deps, org, ownerEmail, step) {
    switch (step) {
 ...
 */
async function reconcileOrganizationProvisioning(orgId, overrides) {
    ...
    return db.transaction(async (trx) => {
        ...
        for (const step of exports.PROVISIONING_STEPS) {
            ...
            if (row?.status === 'done')
                continue;
            try {
                await runStep({ ...deps, db: trx }, org, ownerEmail, step);
        ...
        const allDone = steps.length === exports.PROVISIONING_STEPS.length &&
 ...
 */
async function runInternalProvision(userId, overrides) {
    const { db } = getDeps(overrides);
...
//# sourceMappingURL=organizationProvisioning.js.map
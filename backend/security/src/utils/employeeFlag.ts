 ...
 *   1. widening the monolith's `rootOrgAdmin.ts` root org-admin grant trigger
 *      to also recognize the new EXPLICIT `employee` user-role marker (in
 *      addition to the legacy implicit `roles ~ admin` trigger, which is
 *      unaffected and always active), and
 *   2. surfacing the "Employee" entry in this service's role catalog
 *      (`GET /api/organizations/:id/roles` → `platformRoles`).
 *
 * Mirrors `backend/src/utils/employeeFlag.ts` — this is the security-service
 * copy of the SAME flag (locked name, same default; security is the service
 * that owns the `/roles` catalog endpoint this flag gates).
 *
 * Named for, and shared with, FF-EPIC-17-S9 (the Employee cross-org console)
 * per the epic's flag taxonomy (`docs/planning/epics/
 * EPIC-17-personal-identity-portal-employee-reconciliation.md`).
 *
 * Type: release. Owner: backend-engineer (identity). Default: OFF. Removal
 * criterion: delete this flag once the Employee label + explicit-marker
 * trigger are 100% rolled out and the flag-OFF path is no longer exercised
 * in prod.
 *
 * NOTE: this flag is rollout convenience only — it never replaces a
 * `permit.check` / the ReBAC `org-admin`-on-root grant itself.
 *
 ...
 * utils/rootMembershipFlag.ts) so a missing/unbuilt package degrades to the

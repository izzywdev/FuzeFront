 ...
 * against a database where 014 had succeeded, so this path was never
 * exercised.
 *
 ...
 * (`017_repair_personal_org_over_reclassification.ts`) for restoring rows
 * already damaged by the unconditional window — editing this file does NOT
 * re-run it on a database that already applied migration 015 (knex records
 * applied migrations by name), so this edit protects only databases that
 * have not yet run 015; already-affected production rows need the forward
 * repair migration.
 *
...
export async function up(knex: Knex): Promise<void> {
  ...
  if (rootOrg) {
    ...
    console.error(
      `[015] SKIPPING personal-org reclassify: organization ${ROOT_ORG_ID} does not ` +

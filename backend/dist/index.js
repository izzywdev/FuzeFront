...
try {
    ...
    console.log('📚 Swagger documentation available at http://localhost:' +
        PORT +
        '/api-docs');
}
catch (error) {
    console.warn('⚠️  Swagger documentation not available (packages not installed)');
          ...
          <pre>
{
  ...
  "module": "./App"
    ...
    console.log('📚 Basic API documentation available at http://localhost:' +
        PORT +
        '/api-docs');
}
// Routes
...
async function startServer() {
    try {
        ...
        if (availablePort !== portNumber) {
            console.log(`🔄 Original port ${portNumber} was busy, using port ${availablePort} instead`);
        }
        httpServer.listen(availablePort, () => {
            console.log(`🚀 FuzeFront backend server running on port ${availablePort}`);
        ...
        httpServer.on('error', (err) => {
            if (err.code === 'EADDRINUSE') {
                ...
                console.log('💡 This might happen if another instance is already running');

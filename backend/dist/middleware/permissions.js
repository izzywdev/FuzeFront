...
function requireOrganizationPermission(action) {
  return async (req, res, next) => {
    ...
    try {
      ...
      const hasPermission = await (0,
      permission_check_1.checkOrganizationPermission)(
        req.user.id,
        action,
        organizationId
      )
...
function requireAppPermission(action) {
  return async (req, res, next) => {
    ...
    try {
      ...
      const hasPermission = await (0, permission_check_1.checkAppPermission)(
        req.user.id,
        action,
        appId,
        organizationId
...
function requireUserManagementPermission(action) {
  return async (req, res, next) => {
    ...
    try {
      ...
      const hasPermission = await (0,
      permission_check_1.checkUserManagementPermission)(
        req.user.id,
        action,
        organizationId,
        targetUserId

...
async function bootstrap() {
    ...
    for (const extra of extraDbs) {
        const a = new pg_1.Client({ host, port, user: superUser, password: superPassword, database: 'postgres' });
        await a.connect();
        try {
            ...
            if (exists.rows.length === 0) {
                ...
                try {
                    ...
                    if (e?.code === '42P04' || e?.code === '23505')
                        console.log(`✅ Database ${extra} exists (concurrent)`);
                    else
                        throw e;
                }
            }
        ...
        const ea = new pg_1.Client({ host, port, user: superUser, password: superPassword, database: extra });
        await ea.connect();
        try {

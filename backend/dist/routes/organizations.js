...
function sanitizeInput(data) {
  ...
  return {
    ...
    slug:
      (_b = data.slug) === null || _b === void 0
        ? void 0
        : _b.trim().toLowerCase(),
    type: data.type || 'organization',
    parent_id:
      ((_c = data.parent_id) === null || _c === void 0 ? void 0 : _c.trim()) ||
      null,
    settings:
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (input.parent_id) {
      ...
      const membership = await (0, database_1.default)(
    ...
    Promise.all([
      ...
      (0, permit_1.assignOrganizationRole)(
        req.user.id,
        organizationId,
        'owner'
      ),
...
router.get('/', auth_1.authenticateToken, async (req, res) => {
  try {
      ...
      .leftJoin('organization_memberships', function () {
        this.on(
          ...
          .andOn(
            ...
            '=',
            database_1.default.raw('?', [req.user.id])
          )
          .andOn(
            'organization_memberships.status',
            '=',
            database_1.default.raw('?', ['active'])
          )
      })
      .where(function () {
    ...
    const total = parseInt(
      (totalResult === null || totalResult === void 0
        ? void 0
        : totalResult.total) || '0'
    )
...
router.get(
  ...
  permissions_1.PermissionMiddleware.canReadOrganization,
  async (req, res) => {
    try {
      const { id } = req.params
...
router.put(
  '/:id',
  auth_1.authenticateToken,
  permissions_1.PermissionMiddleware.canUpdateOrganization,
  async (req, res) => {
    var _a
    try {
      const { id } = req.params
...
router.delete(
  '/:id',
  auth_1.authenticateToken,
  permissions_1.PermissionMiddleware.canDeleteOrganization,
  async (req, res) => {
    try {
      const { id } = req.params
      ...
      if (
        parseInt(
          (childOrganizations === null || childOrganizations === void 0
            ? void 0
            : childOrganizations.count) || '0'
        ) > 0
      ) {
        return res.status(400).json({
          error:

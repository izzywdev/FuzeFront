...
exports.__billingProxyConfig = exports.billingWebhookRouter = void 0;
 ...
 */
function readRequestedEntity(req) {
 ...
 */
async function authorizeBillingEntity(req, res, action) {
    ...
    const entityType = requested.entityType === 'organization' ? 'organization' : 'user';
    if (entityType === 'user') {
    ...
    let allowed;
    try {
        allowed = await (0, permission_check_1.checkOrganizationPermission)(userId, action, organizationId);
    }
    catch (err) {
 ...
 */
async function authorizeCheckout(req, res) {
    const userId = req.user?.id;
    if (!userId) {
        res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
        return null;
    ...
    let allowed;
    try {
        allowed = await (0, permission_check_1.checkOrganizationPermission)(userId, 'manage', organizationId);
    }
    catch (err) {
...
const BILLING_SERVICE_URL = (process.env.BILLING_SERVICE_URL || 'http://fuzefront-billing-service:3006').replace(/\/+$/, '');
...
const UPSTREAM_TIMEOUT_MS = parseInt(process.env.BILLING_PROXY_TIMEOUT_MS || '15000', 10);
/**
...
function buildForwardedParams(query) {
    const out = {};
    if (!query)
        return out;
...
async function forward(req, res, options) {
    ...
    const config = {
        ...
        url,
        headers,
        params: buildForwardedParams(req.query),
    ...
    if (data !== undefined)
        config.data = data;
    try {
        const upstream = await axios_1.default.request(config);
        ...
        console.error(`[billing-proxy] upstream error for ${req.method} ${options.path}:`, ax.code || ax.message);
        res
            .status(502)
...
// ---------------------------------------------------------------------------
router.get('/plans', (req, res) => forward(req, res, { path: '/plans', internalAuth: true }));
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
router.post('/subscriptions', auth_1.authenticateToken, async (req, res) => {
...
router.get('/subscriptions/:stripeSubscriptionId', auth_1.authenticateToken, async (req, res) => {
    const entity = await authorizeBillingEntity(req, res, 'read');
    if (!entity)
        return;
    return forward(req, res, {
...
router.patch('/subscriptions/:stripeSubscriptionId', auth_1.authenticateToken, async (req, res) => {
    const entity = await authorizeBillingEntity(req, res, 'manage');
    if (!entity)
        return;
    return forward(req, res, {
...
router.delete('/subscriptions/:stripeSubscriptionId', auth_1.authenticateToken, async (req, res) => {
    const entity = await authorizeBillingEntity(req, res, 'manage');
    if (!entity)
        return;
    return forward(req, res, {
...
router.post('/setup-intent', auth_1.authenticateToken, async (req, res) => {
    const entity = await authorizeBillingEntity(req, res, 'manage');
    if (!entity)
        return;
    return forward(req, res, {
...
// ---------------------------------------------------------------------------
exports.billingWebhookRouter = express_1.default.Router();
exports.billingWebhookRouter.post('/', express_1.default.raw({ type: '*/*', limit: '1mb' }), (req, res, _next) => {

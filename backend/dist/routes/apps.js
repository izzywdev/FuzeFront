...
async function checkAppHealth(app) {
    try {
        ...
        console.log(`Health check failed for ${app.name} (${app.url}):`, errorMessage);
        return false;
    }
}
...
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
        let { name, url, iconUrl, integrationType = 'iframe', remoteUrl, scope, module, description, } = req.body;
        ...
        const existingApp = await (0, database_1.db)('apps').where('name', name).first();
        if (existingApp) {
            return res
                .status(409)

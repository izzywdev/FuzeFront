...
exports.seed = seed;
// applications-service seed — initial demo apps. Rewritten to be IDEMPOTENT:
// no destructive del(); inserts use ON CONFLICT (id) DO NOTHING so re-seeding an
// existing DB is a clean no-op and never clobbers live rows. Dev/bootstrap only.
async function seed(knex) {
    ...
    await knex('apps').insert(rows).onConflict('id').ignore();
    console.log('✅ Apps seeded successfully (idempotent)');
}
//# sourceMappingURL=001_initial_apps.js.map
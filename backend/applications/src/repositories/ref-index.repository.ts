 ...
 *
 * Uses the `app_ref_index` and `app_ref_index_state` tables (public schema).
 * Applications-service references organizationId from create bodies; the org
 * is owned by security-service. The projection allows assertRefExists to check
 * org existence without an RPC to security-service.
 *
...
export class KnexRefIndexRepository implements RefIndexStore {
  ...
  private readonly table = 'app_ref_index'
  private readonly stateTable = 'app_ref_index_state'

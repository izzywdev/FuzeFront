 ...
 *
 * Uses the `ref_index` and `ref_index_state` tables (public schema, monolith).
 * The monolith references userId and orgId from JWTs and request bodies —
 * entities owned by security-service / host backend. The projection allows
 * assertRefExists to answer without an RPC to those services.
 *
...
export class KnexRefIndexRepository implements RefIndexStore {
  ...
  private readonly table = 'ref_index'
  private readonly stateTable = 'ref_index_state'

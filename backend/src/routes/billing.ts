...
// AuthenticatedRequest-like shape. `authenticateToken` populates req.user
// (typed as User from the global Express.Request augmentation); we also stash
// the server-derived, authorized entity for the forwarder to use.
// NOTE: We intentionally do NOT override `user` here — the global augmentation
// in src/types/express.d.ts already types it as `User` (email: string required).
// Overriding with email?: string would make BillingRequest incompatible with
// Request and cause TS2345 at every call site.

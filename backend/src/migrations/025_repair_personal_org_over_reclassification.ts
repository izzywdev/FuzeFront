 ...
 * (2026-08-23, prod). Monolith copy of `backend/security/src/migrations/
 * 017_repair_personal_org_over_reclassification.ts` — kept consistent per
 * the epic's "mirror into the non-live backend copy" DoD.
 *
 * BACKGROUND: migration 022's step (b) reclassified `type='personal'` ->
 ...
 * personal org that migration 022 incorrectly reclassified — never a
...
export async function up(knex: Knex): Promise<void> {
  ...
  for (const row of affected) {
    await knex.transaction(async trx => {
      ...
      if (duplicate) {
        ...
        if (hasContent) {
          console.warn(
            `[025] SKIPPING restore for organization ${current.id} ` +
        ...
        duplicatesRemoved++
        console.log(
          `[025] removed empty duplicate personal org ${duplicate.id} ` +
  ...
  console.log(
    `[025] personal-org over-reclassification repair: restored ${restored} ` +

 ...
 * `provisionPortal` (services/portalProvisioning.ts) can be re-triggered
 * after a mid-step failure and resume from the failed step without
 * re-creating prior resources (AC2), while a Postgres advisory lock
 * (`hashtext(slug)`) serializes concurrent same-slug requests (AC3).
 *
 ...
 *
 * Numbered 018 (not 017 as originally drafted) — 017 was already taken by
 * `017_app_scope_levels_and_installations.ts` when this landed on master.
 */

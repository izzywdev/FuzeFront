 ...
 *
 * Mirrors `backend/security/src/utils/rootMembershipFlag.ts` — this is the
 * monolith copy of the SAME flag (locked name, same default), kept
 * consistent per the epic's "mirror into the non-live backend copy" DoD
 * (security-service is the LIVE/authoritative provisioning path; this copy
 * exists so the two backends never disagree while both still deploy).
 *
 ...
 * `organizationProvisioning.ts` (both copies) once root membership is 100%
 * rolled out and the flag-OFF path is no longer exercised in prod.
 *
 ...
 * utils/portalFlag.ts and utils/identityFlag.ts) so a missing/unbuilt package
 * degrades to the fail-safe default (OFF) rather than crashing route/service

/**
 * Unit tests for backend/src/identity/flags.ts — step 4 of FFRNT-185.
 *
 * Tests both the flag-OFF (default, release flag) and flag-ON paths of
 * isPrefixedIdsEnabled for the main fuzefront-backend slice, per the
 * feature-flags skill's "test BOTH states" rule.
 *
 * No DB, no network — pure unit tests.
 */

// Like all main-backend tests, this file relies on the test database setup in
// tests/setup.ts. In CI it uses PERMIT_API_KEY=ci-no-real-permit-calls and a
// running Postgres instance; locally it is skipped if DB is unreachable (same
// behavior as provisioning.test.ts, rootMembershipBackfillMigration.test.ts etc).
import {
...
describe('isPrefixedIdsEnabled — fuzefront-backend', () => {
  ...
  describe('flag ON', () => {
    ...
    it('returns true with context when client returns true', async () => {
      setIdentityFlagClient(makeClient(true))
      expect(
        await isPrefixedIdsEnabled({ orgId: 'org_01h', userId: 'usr_01h' })
      ).toBe(true)
    })
  })

...
function getDatabaseConfig(options = {}) {
    ...
    if (usePostgres) {
        ...
        console.log(`🔧 Database config: User=${dbUser}, Password type=${typeof dbPassword}, Host=${process.env.DB_HOST || 'localhost'}, migrationsTable=${tableName}`);
...
function initializeDatabaseConnection(options = {}) {
    exports.db = (0, knex_1.knex)(getDatabaseConfig(options));
}
 ...
 */
async function waitForTable(tableName, maxRetries = 60, retryDelay = 2000) {
    ...
    for (let i = 0; i < maxRetries; i++) {
        ...
        console.log(`⏳ Waiting for table "${tableName}"... (attempt ${i + 1}/${maxRetries})`);
        if (i === maxRetries - 1) {
            throw new Error(`Table "${tableName}" did not appear after ${maxRetries} attempts`);
        }
        await new Promise(resolve => setTimeout(resolve, retryDelay));
    }
}
...
async function runSeeds(options = {}) {
    ...
    const seedDb = exports.db ?? (0, knex_1.knex)(getDatabaseConfig(options));
    try {

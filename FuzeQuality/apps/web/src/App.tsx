...
function Requirements({ data }: { data: Portfolio }) {
  ...
  return <><PageHeading eyebrow="Product intent" title="Requirements & inferred flows" detail="Jira stays authoritative. AI proposals remain visibly separate until reviewed." action={<div className={`header-badge freshness-${freshness}`}><Database /> Jira {freshness}</div>} /><div className="requirements-grid">{data.requirements.map(requirement => {
    ...
    return <article className="requirement-card" key={requirement.id}>
      ...
      <details className="requirement-flows"><summary><Network size={14} /> Confirmed actor-step flows</summary>{flows.length ? flows.map(flow => <section className="requirement-flow" key={flow.id}><strong>{flow.title}</strong><small>{flow.origin} · {flow.steps.length} steps</small><ol className="flow-steps">{flow.steps.map(step => <li key={step.id} className={`flow-step-${step.variant}`}><b>{step.position}</b><div><strong>{step.actor}</strong><span>{step.action}</span><small>Expected: {step.expectedOutcome}</small>{step.targetIds.length > 0 && <div className="flow-targets">{step.targetIds.map(target => <code key={target}>{target}</code>)}</div>}</div></li>)}</ol></section>) : <p className="flow-empty">No reviewed flow is attached to this requirement yet. AI proposals remain in the review queue until confirmed.</p>}</details>

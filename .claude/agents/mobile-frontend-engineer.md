...
5. **Wait for Telegram approval.** `design-review-notify.yml` fires automatically and sends the issue link to Telegram. The product owner comments `@fuze approve` (or `@fuze reject: <reason>`) → `claude.yml` spawns a new session to continue.

...
4. **Design-system-first:** use `@fuzeone/design-system` tokens. For missing mobile breakpoint tokens (`--breakpoint-mobile`, `--breakpoint-tablet`) add them to `design-system/base.css` or `design-system/spacing.css` — never hard-code `768px` in component files; reference the token or a CSS custom property.

...
- **database-engineer** — the **data tier**: per-service Postgres/Redis/Mongo/Neo4j/Chroma roles & databases, ordered + idempotent migrations (and how they run on deploy), and connection wiring (`DATABASE_URL`/SealedSecret/service-DNS). Stores are provided by FuzeInfra; this agent owns everything *between* the app and them. NOT app/business logic (that's backend-engineer), NOT deploy charts (devops-engineer), NOT cluster DB operation (FuzeInfra).

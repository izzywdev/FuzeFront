...
4. **Apply**: `node "$TMP/fuzeone-hub/fuzeone/sync.mjs" --target . --scope @fuzeone --repo <Name>`. This stamps the 6 domain agents + README, the `<!-- FUZEONE -->` block in `CLAUDE.md` (merged, not clobbered), `.npmrc`, and the standard workflows (helm-validate / infra-dispatch only if the repo has a chart / declared infra).

...
`accent` accepts hex / `rgb()` / `hsl()` (and their alpha variants). Fail-closed: an unparsable string OR one that cannot reach WCAG 2.1 AA (≥ 4.5:1) against the white on-accent text this DS renders on accent surfaces is **rejected** — the scope then applies no override, so children fall back to the base `@fuzeone/design-system` accent (or an ancestor `BrandTokenScope`, for nesting). `data-brand-accent-status="applied\|fallback"` and `data-brand-fallback-reason` are set for QA/telemetry. `resolveBrandAccent(accent)` is exported standalone for validating a color outside React (e.g. in an admin form).

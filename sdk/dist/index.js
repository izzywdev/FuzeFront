...
function PlatformProvider({ children, config, fallbackMode = false, }) {
    ...
    return (jsxRuntime.jsx(PlatformContext.Provider, { value: { state, dispatch }, children: children }));
}
...
function useGlobalMenu() {
    ...
    const addMenuItem = react.useCallback((item) => {
    ...
    const removeMenuItem = react.useCallback((id) => {
        const newItems = state.menuItems.filter(item => item.id !== id);
        setMenuItems(newItems);
    }, [state.menuItems, setMenuItems]);
    const updateMenuItem = react.useCallback((id, updates) => {
        const newItems = state.menuItems.map(item => item.id === id ? { ...item, ...updates } : item);
        setMenuItems(newItems);
    }, [state.menuItems, setMenuItems]);
    const addAppMenuItems = react.useCallback((appId, items) => {
    ...
    const removeAppMenuItems = react.useCallback((appId) => {
...
function useSocketBus(appId) {
    ...
    const emit = react.useCallback((eventType, payload, targetAppId) => {
...
function useToast() {
    const notify = react.useCallback((toast) => { var _a; return (_a = getBridge()) === null || _a === void 0 ? void 0 : _a.notify(toast); }, []);
    const dismiss = react.useCallback((id) => { var _a; return (_a = getBridge()) === null || _a === void 0 ? void 0 : _a.dismiss(id); }, []);
...
function usePlatform() {
    const [snapshot, setSnapshot] = react.useState(() => { var _a, _b; return (_b = (_a = getBridge()) === null || _a === void 0 ? void 0 : _a.getContext()) !== null && _b !== void 0 ? _b : STANDALONE; });
    react.useEffect(() => {
